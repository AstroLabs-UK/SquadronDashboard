...
const UNIFORM_TAG = /\s*[\[(]\s*uniform\s*[:=-][^\])]*[\])]/ig;   // "[Uniform: Blues]" in a title is shown on the uniform panel instead
const HOUR = 60 * 60 * 1000;

// Keeps events that haven't finished yet. Events with no end time stay visible for 2 hours.
...
function shapeCalendarEvent(e, tz) {
  ...
  return { title, date, detail: parts.join(' · '), source: 'calendar', start: e.start };
}

// A hand-typed event can carry a "hide after" date (YYYY-MM-DD); it disappears the day after it.
function activeManual(events, todayKey) {
  return (Array.isArray(events) ? events : []).filter(e => !(e && e.hideAfter && e.hideAfter < todayKey));
}

/**
 * @param manual   settings.events
 * @param calendar raw calendar events (may be empty)
 * @returns {Array<{title,date,detail,source}>}  hand-typed events first, then the next calendar events
 */
...
module.exports = { buildEventList, upcoming, shapeCalendarEvent, activeManual, UNIFORM_TAG, DAY };
