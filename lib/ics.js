...
function parseCalendar(text, { now = Date.now(), days = 60, pastDays = 1, tz = 'Europe/London' } = {}) {
  ...
  for (const ev of raw) {
    ...
    infos.push({
      ...
      categories: (() => {
        const raw = (first(ev, 'CATEGORIES') || {}).value || '';
        return String(raw).split(',').map(s => plainText(s).trim()).filter(Boolean);
      })()
    });
  }
  ...
  const emit = (info, wall) => {
    ...
    out.push({ uid: info.uid, title: info.title, description: info.description, location: info.location, categories: info.categories || [], allDay: start.allDay, start: s, end: e, startKey, endKey });
  };
